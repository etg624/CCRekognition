...
exports.processInsert = function(connection, connectionEB, connectionAL, caller, csvFileName, callback) {
    ...
    //###### Dev 01 2017 -- Support for different generations of S2 formt
    // Gen 1:  Command, PersonID, Partition, Controller, Credential, FirstName, LastName, PictureFile
    // Gen 2:  Command. PersonID, Partition, FirstName, LastName, Credential, EmployeeID, PictureFile
    var exportFormat = process.env.EXPORT_SOURCE
          ...
          //columns: true
            ...
            if (err) {
                ...
                for (var i=1; i < data.length; i++) {
                  ...
                  //###### Dev 01 2017 -- Support for different generations of S2 formt
                  console.log ("The csvcredential parse " +JSON.stringify(data[i][4]))
                  if (exportFormat == "S2-1"){
                      firstName = data[i][5];
                      lastName = data[i][6];
                      badgeNumber = 0;
                      title = "";
                  ...
                  if (exportFormat == "S2-2"){
                     ...
                     * NOTE: this was changed top reflect fact that there were two export file formats,
                     * each with the names and credentials in different places in the file
                     */
...
//###### Dev 13 2017 -- Support for different generations of S2 formt
    // Gen 1:  Command, PersonID, Partition, Controller, Credential, FirstName, LastName, PictureFile
    // Gen 2:  Command. PersonID, Partition, FirstName, LastName, Credential, EmployeeID, PictureFile
    var exportFormat = process.env.EXPORT_SOURCE
          ...
          //columns: true
            ...
            if (err) {
                ...
                for (var i=1; i < data.length; i++) {
                   ...
                   //###### Wed Dec 13 08:55:38 PST 2017  Support for the alternate S2 formats
                    ...
                    for (var j=0; j < credArr.length; j++) {
                        ...
                        if (credArr[0] ==" " || credArr[0].length ==0){
                          ...
                          status = cardArr[3];
                            ...
                            //###### Wed Dec 13 08:57:20 PST 2017 Adjusted to support alternative S2 format
                            var escapeName = connection.escape(lastName);
                            var escapeFirstName = connection.escape(firstName);
