...
exports.getCardholders = function(token, callback) {
...
        path : '/api/access/onguard/openaccess/instances?type_name=lnl_cardholder&version=1.0',
...
                console.log( "here is the JSON Parsed data : "+JSON.stringify(parsedData) );
                ...
                for (var i=0; i < count; i++) {
                    iD=parsedData.item_list[i].property_value_map.ID
                    lastName=parsedData.item_list[i].property_value_map.LASTNAME
                    firstName=parsedData.item_list[i].property_value_map.FIRSTNAME
                    dept=parsedData.item_list[i].property_value_map.DEPT
                    building=parsedData.item_list[i].property_value_map.BUILDING
                    division=parsedData.item_list[i].property_value_map.DIVISION
                    title=parsedData.item_list[i].property_value_map.TITLE
                    email=parsedData.item_list[i].property_value_map.EMAIL
                    
                    array.push({ID : iD, LastName: lastName, FirstName: firstName, Department : dept, Building : building, Division : division, Title : title, Email : email}); 
                    console.log("here is the array "+JSON.stringify(array))
                    console.log("here is the array length "+JSON.stringify(array.length))
                    console.log("here is an addressable array field "+array[i].LastName)
