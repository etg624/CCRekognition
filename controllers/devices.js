...
//###### Tue Feb 1 06:33:50 PDT 2017 Register the Device with Lenel if being activated
    ...
    db.createConnection(function(err,reslt){  
        if (err) {
          ...
          var connection = reslt;
              ...
              //###### Mon Apr 16 14:07:23 PDT 2018 Added device name to the update SQL
              var _name = req.body.name

              var strSQL = 'UPDATE DeviceHeader SET CurrentStatus='+_currentStatus+', name="'+_name+'" WHERE AuthCode="'+req.params.authCode+'"';
              ...
              var query = connection.query(strSQL, function(err, result) {
                 ...
                 if (err) {
                    ...
                    var _statusDate = datetime.syncCurrentDateTimeforDB(); 
                     ...
                     //###### Tue Feb 1 06:33:50 PDT 2017 Register the Device with Lenel if being activated
                      // Only doing this if being activated, not everytime the statis is changed to whitelist
                      if (process.env.EXPORT_SOURCE == "LENEL_DIRECT"){            
                        var strSQL2 =  'SELECT *  FROM DeviceHistory WHERE AuthCode ="'+_authCode+'" AND Status = "1"';
                        console.log('INSERT strSQL2= '+ strSQL2);  
    
                          connection.query(strSQL2, function(err, rows) {
                                if (err) {
                                  console.log('results of devicehistory query '+err);
                                  
                                }else{
                                  //if (rows == 0){
                                    //Device has not previously been activated and therefore has not been registered with Lenel
                                    lnlAddDevice.addDevice(_authCode, function(err, result){ 
                                      if (!err){ console.log("DEVICE "+_authCode+" successfully added to Lenel")}
