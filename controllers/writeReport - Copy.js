...
//###### Sat Oct 28 18:28:20 CDT 2017  TEmpID added as a parm and for passing to the attendance report
      ...
      db.createConnection(function(err,reslt){  
          if (err) {
            ...
            if (report == 'Attendance') {
              var eventID = itemID;
              //###### Sat Feb 25 18:28:20 CDT 2017  Added a timestamp to report so that it can be run multiple times for 
              //an event.  Neeed to pass the name back through the callbacks
              module.exports.writeAttendanceReport (connection, eventID, itemTempID, function(err,reslt, fileName){  
            ...
            if (report == 'Mustering') {
              ...
              module.exports.writeMusteringReport (connection, musterID, function(err,reslt){  
                callback(sess.rptError, sess.rptSuccess);
                return;
...
module.exports.writeAttendanceReport = function(connection, eventID, eventTempID, callback) {
        ...
        if (eventTempID =="") {
          ...
          var _sqlQ = 'SELECT * FROM attendance WHERE EventID='+eventID+' or EventID="'+eventTempID+'"'
            ...
            connection.query(_sqlQ, function(err, results) {
              ...
              if(results.length > 0){
                ...
                //###### Tue Feb 23 09:36:46 PST 2017 - Use time stamp so many copies of same report can be run
                ...
                fs.open(rptPath+title+'.csv', 'wx', (err, fd) => {
                  if (err) {
                    if (err.code === "EEXIST") {
                      ...
                      connection.end();
                      //###### Sat Feb 25 18:28:20 CDT 2017  Added a timestamp to report so that it can be run multiple times 
          ...
          //###### Tue Feb 27 09:36:46 PST 2017 - Use attendanceReport table to format the report
          ...
          // Process for formatting the report:
          // 1. Read the attendanceReport table into an array
          // 2. Loop through the array removing all entries where the value is "NO"
          // 3. Format the header line from the remaining Keys and the write line in the same way
          // 4. For the write line, need a way to lnow which result field to address and how.  stringify the results?
          // move them into an array?  Read all the results into variales with the same names as attendance report
          //Object.keys(jsonObj);
                ...
                fs.close(fd, function(err){         
                  ...
                  //###### Tue Dec 5 08:45:10 PST 2017  FTP to download the report to the local user machine                       
                  // var c = new client();
                   // c.on('ready', function() {
...
// Report of all Attendance for an Event //
///////////////////////////////////////////
module.exports.writeMusteringReport = function(connection, eventID, callback) {
