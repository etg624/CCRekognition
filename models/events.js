...
exports.getAllEventsEndedSinceLastSweep = function(connection, callback) {
  ...
  var strSQL = "Select * from events where EventDateTime <= '"+currentDateTime+"' AND EventDateTime > '"+oneMonthAgoDateTime+"' AND EventEndProcessed !='YES' AND EventsType != 'Mustering'"
  //
...
//###### Sun Jun 10 09:51:51 PDT 2018 Support for Event End monitoring
//////////////////////////////////////////////////////////////////////////////////////////////////////////
//  Mark Event record as ended and processed if there was attendance and report was emailed to event admin
//////////////////////////////////////////////////////////////////////////////////////////////////////////

exports.markEventAsEndedAndProcessed = function(eventsForWhichReportsWereSent, callback) {
  console.log('IN MARK '+ eventsForWhichReportsWereSent); 
  ...
  db.createConnection(function(err,reslt){ 
    ...
    if (err) {
      ...
      for (var i=0; i < eventsForWhichReportsWereSent.length; i++){

        var strSQL = "UPDATE events SET EventEndProcessed = 'YES' WHERE EventID="+eventsForWhichReportsWereSent[i]
        //
        console.log('Event End strSQL= '+ strSQL); 
        //
        var query = connection.query(strSQL, function(err, result) {
          ...
          if (err) {
            console.log("Event End update error :" +err)
            callback(err, null);
          } else {  
            
            if (counter==eventsForWhichReportsWereSent.length){
              console.log("UPDATE END LOGIC")
              callback(null, counter+" EVENTS updated as ended");
            }
          }
