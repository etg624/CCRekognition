				...
				if (err) {
					...
					if (allEventsSinceLastSweep.length > 0) {
								...
								for (var i=0 ; i < allEventsSinceLastSweep.length; i++){ 
									...
									writeReport.writeActivityReport ('eventEndSweep','Attendance', connection, allEventsSinceLastSweep[i].EventID, allEventsSinceLastSweep[i].TempID, function(err, result, fileNameofReport){  
